struct MyUniform {
	color: vec4f,
	time: f32,
};

@group(0) @binding(0) var<uniform> uMyUniform: MyUniform;

struct VertexInput {
	@location(0) position: vec3f,
	@location(1) color: vec3f,
}

struct VertexOutput {
	@builtin(position) position: vec4f,
	@location(0) color: vec3f,
}

@vertex
fn vs_main(in: VertexInput) -> VertexOutput {
	var offset = vec2f(-0.0, -0.0);
	offset += 0.3 * vec2f(cos(uMyUniform.time), sin(uMyUniform.time));

	let alpha = cos(uMyUniform.time);
	let beta = sin(uMyUniform.time);
	
	var out: VertexOutput;
	out.position = vec4f(
		in.position.x + offset.x,
		alpha * in.position.y + beta * in.position.z + offset.y,
		alpha * in.position.z - beta * in.position.y * 0.5 + 0.5, 1.0);
	out.color = in.color;
	return out;
}

@fragment
fn fs_main(in: VertexOutput) -> @location(0) vec4f {
	let color = in.color * uMyUniform.color.rgb;
	let linear_color = pow(color, vec3f(2.2));
	return vec4f(linear_color, 1.0);
} 